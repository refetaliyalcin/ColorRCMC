...
pigment2_ro=12*10^-9;
pigment2_ri=9*10^-9;
pigment2_r=pigment2_ro;
e_pigment_=n_pigment.^2-k_pigment.^2; % eq. 2.30
e_pigment__=n_pigment.*k_pigment*2;
e_pigment=e_pigment_+e_pigment__*1i;

r_eff_2=(4/3)*(pigment2_ro^3-pigment2_ri^3)/(pigment2_ro^2+pigment2_ri^2);
delta_e_free=0;
for n=1:100
...
for k=1:length(f_v_1)
    for j=1:length(f_v_2)
        ...
        for i=1:length(lamda)
            ...
            x_2=2*pi*n_medium_sc*pigment2_ri/lamda(i);
            y_2=2*pi*n_medium_sc*pigment2_ro/lamda(i);
            m1_2=n_sio2(i)/n_medium_sc;
            m2_2=(n_new_pigment_2(i)+1i*k_new_pigment_2(i))/n_medium_sc;
            fonksiyon_2=Miecoated(m1_2,m2_2,x_2,y_2,1);
